...
from .eqns import Eqn, Eqn_For_Species
...
def _read_csv(fp, specie_only):
    ...
    If specie_only, k won't be read
    ...
    specie_only
        boolean for specie-extraction mode
    ...
    pdt_arr = eqn_arr[:, 1]
    if not specie_only:
        k_arr = eqn_arr[:, 2].astype(np.float64)

    eqns = []
    
    if specie_only:
        for rcts, pdts in zip(rct_arr, pdt_arr):
            ms_rcts = parse_eqn_side(rcts)
            ms_pdts = parse_eqn_side(pdts)

            eqns.append(Eqn_For_Species(ms_rcts, ms_pdts))
...
def read_csvs(fp_list, specie_only):
    ...
    specie_only: boolean
        Switch for specie-extraction mode
    ...
    eqns0 = []
    for fp in fp_list:
        eqns0.extend(_read_csv(fp, specie_only))
